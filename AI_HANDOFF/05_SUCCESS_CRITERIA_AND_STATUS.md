> This file is the objective, itemized, already-verified success checklist for this MVP, copied from the project root's `SUCCESS_CRITERIA.md` at the time this handoff package was written. Every checked item was verified by actually driving the running app with Playwright and real API calls — not by inspection or by trusting the model's own claims. Use this as the acceptance bar for a rebuild: a faithful reproduction should be able to pass the same checks the same way.
